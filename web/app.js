...
function render(data) {
  ...
  function showEvents() {
    ...
    row.className = status; row.append(cell(event.date), cell(event.start_time), cell(event.series), cell(event.location), cell(event.home_team), cell(event.away_team), cell(labels[status]));
    ...
    row.append(cell(details || "–")); body.append(row);
    }
  }
  filter.addEventListener("change", showEvents); showEvents();
}
fetch("data/matches.json").then(response => { if (!response.ok) throw new Error("Kunde inte läsa matchdata"); return response.json(); }).then(render).catch(error => { const notice = document.querySelector("#error"); notice.hidden = false; notice.textContent = error.message; });
